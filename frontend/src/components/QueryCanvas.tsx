...
import { generateAlias, generateNodeId, generateJoinId } from '@/lib/utils';
...
export default function QueryCanvas({ onDrop, onDragOver }: QueryCanvasProps) {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const existingAliases = useQueryStore.getState().tables.map(t => t.alias);
      const newTable: TableNodeType = {
        id: generateNodeId(),
        tableName,
        alias: generateAlias(tableName, existingAliases),
        position,
      };
